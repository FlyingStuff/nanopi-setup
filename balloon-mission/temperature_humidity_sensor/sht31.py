...
class SHT31:
    ...
    def __init__(self, device_number=1, address=0x44):
        """Opens the i2c device (assuming that the kernel modules have been
        loaded)."""
        self.i2c = open('/dev/i2c-%s' % device_number, 'r+b', 0)
        fcntl.ioctl(self.i2c, self._I2C_SLAVE, address)
        time.sleep(0.050)
...
if __name__ == "__main__":
    import sys
    sht31 = SHT31(sys.argv[1], 0x44)
    print (sht31.check_heater_status())
    # try:
    #     with SHT31(sys.argv[1]) as sht31:
    #         print sht31.check_heater_status()
